...
export class QueueService {
  constructor(
    ...
    @InjectQueue(QUEUE_NAMES.RESOURCE_CURATION) private resourceQueue: Bull.Queue,
    @InjectQueue(QUEUE_NAMES.EXERCISE_GENERATION) private exerciseQueue: Bull.Queue,
    @InjectQueue(QUEUE_NAMES.NOTIFICATION) private notificationQueue: Bull.Queue,
  ) {}
  ...
  async addPathGenerationJob(payload: PathGenerationPayload) {
    return this.pathQueue.add(
      JOB_NAMES.GENERATE_PATH,
      payload,
      {
  ...
  async addResourceCurationJob(payload: ResourceCurationPayload) {
    return this.resourceQueue.add(
      JOB_NAMES.CURATE_MODULE_RESOURCES,
      payload,
      {
        attempts: 3,
